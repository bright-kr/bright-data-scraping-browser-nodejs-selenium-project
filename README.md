# Bright Data Scraping Browser Nodejs Selenium Project

[![Bright Data Promo](https://github.com/bright-kr/LinkedIn-Scraper/raw/main/Proxies%20and%20scrapers%20GitHub%20bonus%20banner.png)](https://brightdata.co.kr/)

<a href="https://codesandbox.io/p/devbox/github/bright-kr/bright-data-scraping-browser-nodejs-selenium-project?file=%2Findex.js" target="_blank" rel="noopener">CodeSandbox에서 열기</a>, GitHub 계정으로 로그인한 다음, 변경을 시작하기 위해 리포지토리를 fork하십시오.

이 프로젝트는 Selenium WebDriver를 사용하여 웹사이트에서 데이터를 스크레이핑하기 위해 <a href="https://brightdata.co.kr/products/scraping-browser" target="_blank" rel="noopener">Bright Data Scraping Browser</a>를 사용하는 방법을 보여줍니다.

## Getting Started

1. `index.js`에서 `YOUR_BRIGHT_DATA_SCRAPING_BROWSER_ENDPOINT` 값을 실제 Bright Data scraping browser 엔드포인트로 교체하십시오.
2. `"https://example.com`을 대상 URL로 교체하십시오.
3. 의존성을 설치하려면 `npm install`을 실행하십시오.
4. 스크레이핑을 시작하려면 `node index.js`를 실행하십시오.

## Demo
https://github.com/user-attachments/assets/5c8f73c1-a848-476d-80c6-a85f55762ce0

## Examples
1. [Booking.com Hotel Scraping Demo](hotel-scraping/README.md)
2. [Amazon Product Scraping Demo](ecommerce-scraping/README.md)